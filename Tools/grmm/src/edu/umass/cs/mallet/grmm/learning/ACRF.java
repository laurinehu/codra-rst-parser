  ...
  /**
    ...
    private int initDenseWeights (InstanceList training)
    {
      ...
      for (int i = 0; i < size; i++) {
        ...
        total += numf;
//        logger.info ("ACRF template "+this+" weights ["+i+"] num features "+numf); //joty
      }

//      logger.info ("ACRF template "+this+" total num weights = "+total); //joty
      weights = newWeights;
...
//TODO:	public static class UnrolledGraph extends FactorGraph
	...
	public List getBestLabels (InstanceList lst)
	{
		...
		for (int i = 0; i < lst.size(); i++) {
			...
			System.out.println(); //added by joty
		}
		return ret;
	}
  ...
  transient private boolean warnOnNoMax = true;
	...
	public LabelsSequence getBestLabels (Instance inst)
	{
		...
		if (unrolled.numVariables () != 0) {
			...
			globalInferencer.computeMarginals (unrolled); // joty
		}
    ...
    // And change it into a LabelsSequence
		...
		for (int t = 0; t < maxTime; t++) {
			...
			Variable v1 = unrolled.lookupVarForLabel (t, 0); //joty
			Variable v2 = unrolled.lookupVarForLabel (t, 1); //joty			
			Variable[] vars = new Variable[] { v1, v2 }; //joty
			Factor marg1 = globalInferencer.lookupMarginal (new HashVarSet (vars)); //joty


         for (AssignmentIterator it = marg1.assignmentIterator (); it.hasNext (); ) {
         	double marginal = marg1.value (it);
         	int idx = it.indexOfCurrentAssn ();
         	
         	System.out.print(idx);
         	System.out.print(":");
         	System.out.print(marginal);
         	System.out.print(" ");
...
//				logger.info ("getValue() (loglikelihood) = " + cachedValue); //joty
	    }
